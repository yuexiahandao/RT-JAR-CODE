...
/*  58 */   private static final Object[][] resource = { { "VERSION", "{0}バージョン{1}" }, { "BAD_ARGUMENT", "無効な引数: {0}" }, { "BAD_OPTION", "無効なオプション: {0}={1}" }, { "BAD_REPACK_OUTPUT", "無効な--repack出力: {0}" }, { "DETECTED_ZIP_COMMENT", "検出されたZIPコメント: {0}" }, { "SKIP_FOR_REPACKED", "すでに再圧縮されているためスキップしています: {0}" }, { "WRITE_PACK_FILE", "*.packファイルを書き込むには、--no-gzipを指定します: {0}" }, { "WIRTE_PACKGZ_FILE", "*.pack.gzファイルを書き込むには、--gzipを指定します: {0}" }, { "SKIP_FOR_MOVE_FAILED", "移動が失敗したため解凍をスキップしています: {0}" }, { "PACK_HELP", { "使用方法:  pack200 [-opt... | --option=value]... x.pack[.gz] y.jar", "", "圧縮オプション", "  -g、--no-gzip                   プレーンな*.packファイルを圧縮せずに出力します", "  --gzip                          (デフォルト)圧縮出力をgzipで後処理します", "  -G、--strip-debug               圧縮中にデバッグ属性を削除します", "  -O、--no-keep-file-order        ファイルの順序付け情報を転送しません", "  --keep-file-order               (デフォルト)入力ファイルの順序付けを保持します", "  -S{N}、--segment-limit={N}       セグメント制限を出力します(デフォルトN=1Mb)", "  -E{N}、--effort={N}             圧縮の試行(デフォルトN=5)", "  -H{h}、--deflate-hint={h}       デフレート・ヒントを転送します: true、falseまたはkeep(デフォルト)", "  -m{V}、--modification-time={V}  変更時間を転送します: latestまたはkeep(デフォルト)", "  -P{F}、--pass-file={F}          指定された圧縮されていない入力要素を転送します", "  -U{a}、--unknown-attribute={a}  不明の属性アクション: error、stripまたはpass(デフォルト)", "  -C{N}={L}、--class-attribute={N}={L}  (ユーザー定義属性)", "  -F{N}={L}、--field-attribute={N}={L}  (ユーザー定義属性)", "  -M{N}={L}、--method-attribute={N}={L} (ユーザー定義属性)", "  -D{N}={L}、--code-attribute={N}={L}   (ユーザー定義属性)", "  -f{F}、--config-file={F}        Pack200.PackerプロパティにファイルFを読み込みます", "  -v, --verbose                   increase program verbosity", "  -q, --quiet                     set verbosity to lowest level", "  -l{F}, --log-file={F}           output to the given log file, or '-' for System.out", "  -?, -h, --help                  print this message", "  -V, --version                   print program version", "  -J{X}                           pass option X to underlying Java VM", "", "注意:", "  -P、-C、-F、-Mおよび-Dオプションは累積されます。", "  属性定義の例:  -C SourceFile=RUH .", "  Config.ファイル・プロパティは、Pack200 APIによって定義されます。", "  -S、-E、-H、-m、-Uの値の意味は、Pack200 APIを参照してください。", "  レイアウト定義(RUHなど)はJSR 200によって定義されます。", "", "再圧縮モードでは、JARファイルが圧縮/解凍サイクルで更新されます:", "    pack200 [-r|--repack] [-opt | --option=value]... [repackedy.jar] y.jar\n" } }, { "UNPACK_HELP", { "使用方法:  unpack200 [-opt... | --option=value]... x.pack[.gz] y.jar\n", "", "解凍オプション", "  -H{h}、--deflate-hint={h}     転送されたデフレート・ヒントをオーバーライドします: true、falseまたはkeep(デフォルト)", "  -r、--remove-pack-file        解凍後に入力ファイルを削除します", "  -v, --verbose                   increase program verbosity", "  -q, --quiet                     set verbosity to lowest level", "  -l{F}, --log-file={F}           output to the given log file, or '-' for System.out", "  -?, -h, --help                  print this message", "  -V, --version                   print program version", "  -J{X}                           pass option X to underlying Java VM" } }, { "MORE_INFO", "(詳細は、{0} --helpを実行してください。)" }, { "DUPLICATE_OPTION", "重複オプション: {0}" }, { "BAD_SPEC", "{0}の無効な仕様: {1}" } };
/*     */ 
 ...
 * Qualified Name:     com.sun.java.util.jar.pack.DriverResource_ja
