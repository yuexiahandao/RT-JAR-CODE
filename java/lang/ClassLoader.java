...
/*      */ {
/*  264 */
        this.classes.addElement(paramClass);
/*      */
    }
...
/*  464 */
            localObject1 = this.parallelLockMap.putIfAbsent(paramString, localObject2);
/*  465 */
...
/*  501 */
                AccessController.doPrivileged(new PrivilegedAction() {
                    ...
                    public Void run() {
/*  503 */
                        localSecurityManager.checkPackageAccess(this.val$name.substring(0, i));
/*  504 */
                        return null;
/*      */
                    }
...
/*      */ 
/*  666 */
        if (paramString != null) checkCerts(paramString, paramProtectionDomain.getCodeSource());
/*      */ 
/*  668 */
...
/*      */ {
/*  689 */
        ClassFileTransformer[] arrayOfClassFileTransformer1 = ClassFileTransformer.getTransformers();
/*      */ 
/*  691 */
        Class localClass = null;
/*      */ 
/*  693 */
        if (arrayOfClassFileTransformer1 != null) {
/*  694 */
            for (ClassFileTransformer localClassFileTransformer : arrayOfClassFileTransformer1) {
/*      */
                try
/*      */ {
/*  697 */
                    byte[] arrayOfByte = localClassFileTransformer.transform(paramArrayOfByte, paramInt1, paramInt2);
/*  698 */
                    localClass = defineClass1(paramString1, arrayOfByte, 0, arrayOfByte.length, paramProtectionDomain, paramString2);
/*      */
                }
...
/*      */ {
/*  800 */
            localClass = defineClass1(paramString, paramArrayOfByte, paramInt1, paramInt2, paramProtectionDomain, str);
/*      */
        } catch (ClassFormatError localClassFormatError) {
/*  802 */
            localClass = defineTransformedClass(paramString, paramArrayOfByte, paramInt1, paramInt2, paramProtectionDomain, localClassFormatError, str);
/*      */
        }
/*      */ 
/*  806 */
...
/*      */ {
/*  898 */
            localObject = defineClass2(paramString, paramByteBuffer, paramByteBuffer.position(), i, paramProtectionDomain, str);
/*      */
        }
/*      */ catch (ClassFormatError localClassFormatError) {
/*  901 */
...
/*  903 */
            localObject = defineTransformedClass(paramString, arrayOfByte, 0, i, paramProtectionDomain, localClassFormatError, str);
/*      */
        }
/*      */ 
/*  907 */
        postDefineClass((Class) localObject, paramProtectionDomain);
/*  908 */
        return localObject;
/*      */
    }
...
/*  924 */ return true;
/*  925 */
        if ((paramString.indexOf('/') != -1) || ((!VM.allowArraySyntax()) && (paramString.charAt(0) == '[')))
/*      */ {
/*  927 */
            return false;
/*  928 */
...
/*      */ {
/* 1068 */
        if (!checkName(paramString)) return null;
/*      */ 
/* 1070 */
...
/*      */ {
/* 1179 */
        Enumeration[] arrayOfEnumeration = new Enumeration[2];
/* 1180 */
        if (this.parent != null)
...
/*      */
        do {
/* 1520 */
            localClassLoader = localClassLoader.parent;
/* 1521 */
...
/*      */ 
/* 1632 */
            this.packages.put(paramString1, localPackage);
/* 1633 */
            return localPackage;
/*      */
        }
...
/* 1651 */
        synchronized (this.packages) {
/* 1652 */
            localObject1 = (Package) this.packages.get(paramString);
/*      */
        }
/* 1654 */
        if (localObject1 == null) {
...
/* 1661 */
                synchronized (this.packages) {
/* 1662 */
                    Package localPackage = (Package) this.packages.get(paramString);
/* 1663 */
                    if (localPackage == null)
...
/* 1685 */
        synchronized (this.packages) {
/* 1686 */
            localHashMap = new HashMap(this.packages);
/*      */
        }
/*      */ 
/* 1689 */
...
/* 1890 */
        if (loadLibrary1(paramClass, paramFile)) {
/* 1891 */
            return true;
/*      */
        }
/* 1893 */
        File localFile = ClassLoaderHelper.mapAlternativeName(paramFile);
/* 1894 */
        if ((localFile != null) && (loadLibrary1(paramClass, localFile))) {
/* 1895 */
            return true;
/*      */
        }
...
/*      */ {
            ...
            public Object run() {
/* 1904 */
                return this.val$file.exists() ? Boolean.TRUE : null;
/*      */
            }
/*      */
        }) != null ? 1 : 0;
...
/*      */ 
/* 1918 */
        Vector localVector = localClassLoader != null ? localClassLoader.nativeLibraries : systemNativeLibraries;
/*      */ 
/* 1920 */
...
/* 1924 */
                if (str.equals(localNativeLibrary1.name)) {
/* 1925 */
                    return true;
/*      */
                }
...
/* 1951 */
                    if (str.equals(localNativeLibrary3.name)) {
/* 1952 */
                        if (localClassLoader == localNativeLibrary3.fromClass.getClassLoader()) {
/* 1953 */
                            return true;
/*      */
                        }
...
/*      */ 
/* 1962 */
                NativeLibrary localNativeLibrary2 = new NativeLibrary(paramClass, str);
/* 1963 */
                nativeLibraryContext.push(localNativeLibrary2);
/*      */
                try {
/* 1965 */
                    localNativeLibrary2.load(str);
/*      */
                } finally {
/* 1967 */
...
/* 1969 */
                if (localNativeLibrary2.handle != 0L) {
/* 1970 */
...
/*      */ {
/* 1981 */
        Vector localVector = paramClassLoader != null ? paramClassLoader.nativeLibraries : systemNativeLibraries;
/*      */ 
/* 1983 */
        synchronized (localVector) {
/* 1984 */
...
/*      */ {
/* 2171 */
            Boolean localBoolean = (Boolean) this.classAssertionStatus.get(paramString);
/* 2172 */
...
/* 2177 */
            if (i < 0) {
/* 2178 */
                localBoolean = (Boolean) this.packageAssertionStatus.get(null);
/* 2179 */
                if (localBoolean != null)
...
/* 2184 */
                localBoolean = (Boolean) this.packageAssertionStatus.get(paramString);
/* 2185 */
                if (localBoolean != null)
...
/*      */ 
/* 2204 */
        for (int i = 0; i < localAssertionStatusDirectives.classes.length; i++) {
/* 2205 */
            this.classAssertionStatus.put(localAssertionStatusDirectives.classes[i], Boolean.valueOf(localAssertionStatusDirectives.classEnabled[i]));
/*      */
        }
/*      */ 
/* 2208 */
        for (i = 0; i < localAssertionStatusDirectives.packages.length; i++) {
/* 2209 */
            this.packageAssertionStatus.put(localAssertionStatusDirectives.packages[i], Boolean.valueOf(localAssertionStatusDirectives.packageEnabled[i]));
/*      */
        }
/*      */ 
/* 2212 */
        this.defaultAssertionStatus = localAssertionStatusDirectives.deflt;
/*      */
    }
...
/* 1769 */
                    for (int j = 0; j < i; j++) {
/* 1770 */
                        if (this.name.equals(ClassLoader.loadedLibraryNames.elementAt(j))) {
/* 1771 */
...
/*  231 */
            synchronized (loaderTypes) {
/*  232 */
                return loaderTypes.contains(paramClass);
/*      */
            }
/*      */
        }
