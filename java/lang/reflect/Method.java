...
/*     */ {
/* 151 */
        Method localMethod = new Method(this.clazz, this.name, this.parameterTypes, this.returnType, this.exceptionTypes, this.modifiers, this.slot, this.signature, this.annotations, this.parameterAnnotations, this.annotationDefault);
/*     */ 
/* 154 */
        localMethod.root = this;
/*     */ 
/* 156 */
        localMethod.methodAccessor = this.methodAccessor;
/* 157 */
        return localMethod;
/*     */
    }
...
/*     */
        Type[] arrayOfType;
/* 336 */
        if ((getGenericSignature() != null) && ((arrayOfType = getGenericInfo().getExceptionTypes()).length > 0))
/*     */ {
/* 338 */
            return arrayOfType;
/*     */
...
/*     */ {
/* 354 */
                if (!this.returnType.equals(localMethod.getReturnType())) {
/* 355 */
                    return false;
/*     */
                }
...
/* 358 */
                Class[] arrayOfClass2 = localMethod.parameterTypes;
/* 359 */
...
/* 405 */
            int i = getModifiers() & Modifier.methodModifiers();
/* 406 */
            if (i != 0) {
/* 407 */
                localStringBuilder.append(Modifier.toString(i)).append(' ');
/*     */
            }
/* 409 */
            localStringBuilder.append(Field.getTypeName(getReturnType())).append(' ');
/* 410 */
            localStringBuilder.append(Field.getTypeName(getDeclaringClass())).append('.');
/* 411 */
...
/* 414 */
                localStringBuilder.append(Field.getTypeName(arrayOfClass1[j]));
/* 415 */
...
/* 423 */
                    localStringBuilder.append(arrayOfClass2[k].getName());
/* 424 */
...
/* 482 */
                localStringBuilder.append('<');
/* 483 */
                for (Object localObject2 : arrayOfTypeVariable) {
/* 484 */
                    if (j == 0) {
/* 485 */
                        localStringBuilder.append(',');
...
/* 495 */
            localStringBuilder.append((localType instanceof Class) ? Field.getTypeName((Class) localType) : localType.toString()).append(' ');
/*     */ 
/* 499 */
            localStringBuilder.append(Field.getTypeName(getDeclaringClass())).append('.');
/* 500 */
...
/* 502 */
            for (???=0;???<???.length;???++){
/* 503 */
                String str = ( ???[???]instanceof Class)?Field.getTypeName((Class) ? ??[???]):???[???].toString();
/*     */ 
/* 506 */
                if ((isVarArgs()) && ( ???==???.length - 1))
/* 507 */
...
/* 517 */
                    localStringBuilder.append((arrayOfType[n] instanceof Class) ? ((Class) arrayOfType[n]).getName() : arrayOfType[n].toString());
/*     */ 
/* 520 */
                    if (n < arrayOfType.length - 1)
...
/*     */ {
/* 598 */
            localObject = getCallerClass();
/* 599 */
            checkAccess((Class) localObject, this.clazz, paramObject, this.modifiers);
/*     */
        }
/*     */ 
/* 602 */
        Object localObject = this.methodAccessor;
/* 603 */
        if (localObject == null) {
/* 604 */
...
/* 606 */
        return ((MethodAccessor) localObject).invoke(paramObject, paramArrayOfObject);
/*     */
    }
...
/* 664 */
        if (this.root != null) localMethodAccessor = this.root.getMethodAccessor();
/* 665 */
        if (localMethodAccessor != null) {
/* 666 */
            this.methodAccessor = localMethodAccessor;
/*     */
        }
/*     */
        else {
...
/*     */ 
/* 741 */
        Object localObject = AnnotationParser.parseMemberValue(localClass, ByteBuffer.wrap(this.annotationDefault), SharedSecrets.getJavaLangAccess().getConstantPool(getDeclaringClass()), getDeclaringClass());
/*     */ 
/* 746 */
