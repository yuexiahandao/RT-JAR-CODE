...
/* 130 */
            localObject1 = (String) this.type;
/* 131 */
            localObject2 = MethodType.fromMethodDescriptorString((String) localObject1, getClassLoader());
/* 132 */
            this.type = localObject2;
/* 133 */
            return localObject2;
/*     */
        }
...
/* 136 */
            localObject1 = (Object[]) this.type;
/* 137 */
            localObject2 = (Class[]) localObject1[1];
/* 138 */
            Class localClass = (Class) localObject1[0];
/* 139 */
            MethodType localMethodType = MethodType.methodType(localClass, (Class[]) localObject2);
/* 140 */
...
/* 228 */
        if (b == 0) return isType();
/* 229 */
        if (isField()) {
/* 230 */
            assert (staticIsConsistent());
/* 231 */
            if ((!$assertionsDisabled) && (!MethodHandleNatives.refKindIsField(b))) throw new AssertionError();
/*     */
        }
/* 232 */
        else if (isConstructor()) {
/* 233 */
            if ((!$assertionsDisabled) && (b != 8) && (b != 7)) throw new AssertionError();
/*     */
        }
/* 234 */
        else if (isMethod()) {
...
/* 246 */
        if (this.clazz == Object.class) return true;
/* 247 */
        MethodType localMethodType = getMethodType();
/* 248 */
        if ((this.name.equals("toString")) && (localMethodType.returnType() == String.class) && (localMethodType.parameterCount() == 0))
/* 249 */ return true;
/* 250 */
        if ((this.name.equals("hashCode")) && (localMethodType.returnType() == Integer.TYPE) && (localMethodType.parameterCount() == 0))
/* 251 */ return true;
/* 252 */
        if ((this.name.equals("equals")) && (localMethodType.returnType() == Boolean.TYPE) && (localMethodType.parameterCount() == 1) && (localMethodType.parameterType(0) == Object.class))
/* 253 */ return true;
/* 254 */
        return false;
/*     */
...
/* 271 */
        ...
        if (!$assertionsDisabled) throw new AssertionError(this);
/* 272 */
        return true;
/*     */
    }
...
/* 286 */
            assert (l >= 0L) : (l + ":" + this);
/* 287 */
            if ((!$assertionsDisabled) && (!(localObject2 instanceof Class))) throw new AssertionError();
/*     */
        }
/* 289 */
        else {
...
/* 290 */
                if ((!$assertionsDisabled) && (l < 0L)) throw new AssertionError(l + ":" + this);
/*     */
            }
/*     */
            else
...
/* 310 */
        return (this.flags & paramInt1) == paramInt2;
/*     */
    }
...
/*     */ {
/* 441 */
        return VerifyAccess.isMemberAccessible(getDeclaringClass(), getDeclaringClass(), this.flags, paramClass, 15);
/*     */
    }
...
/* 532 */
        byte b = getReferenceKind();
/* 533 */
        if (!$assertionsDisabled) if (b != (isStatic() ? 2 : 1)) throw new AssertionError();
/* 534 */
        if (paramBoolean)
...
/* 577 */
        localMemberName.clazz = null;
/* 578 */
        localMemberName.type = null;
/* 579 */
        localMemberName.name = null;
/* 580 */
        localMemberName.resolution = localMemberName;
/* 581 */
...
/*     */ {
/* 588 */
        return Objects.hash(new Object[]{this.clazz, Integer.valueOf(this.flags), this.name, getType()});
/*     */
    }
...
/*     */ {
/* 601 */
        if (this == paramMemberName) return true;
/* 602 */
        if (paramMemberName == null) return false;
/* 603 */
        return (this.clazz == paramMemberName.clazz) && (this.flags == paramMemberName.flags) && (Objects.equals(this.name, paramMemberName.name)) && (Objects.equals(getType(), paramMemberName.getType()));
/*     */
    }
...
/*     */ {
/* 624 */
        this(paramClass1, paramString, paramClass2, (byte) 0);
/* 625 */
        initResolved(false);
/*     */
    }
...
/* 678 */
            if (((MethodType) localObject).erase() == localObject) return;
/* 679 */
            if (VerifyAccess.isTypeVisible((MethodType) localObject, this.clazz)) return;
/* 680 */
            throw new LinkageError("bad method type alias: " + localObject + " not visible from " + this.clazz);
/*     */
        }
/*     */ 
/* 683 */
...
/* 687 */
        if (VerifyAccess.isTypeVisible((Class) localObject, this.clazz)) return;
/* 688 */
        throw new LinkageError("bad field type alias: " + localObject + " not visible from " + this.clazz);
/*     */
    }
...
/* 708 */
            localStringBuilder.append(getName(this.clazz));
/* 709 */
...
/* 736 */
        if (paramObject != null) paramString = paramString + ", from " + paramObject;
/* 737 */
...
/* 811 */
                if (localArrayList1 == null) localArrayList1 = new ArrayList(1);
/* 812 */
...
/* 820 */ for (localIterator = localArrayList1.iterator(); localIterator.hasNext(); ) {
...
/* 848 */
                localMemberName.checkForTypeAlias();
/* 849 */
                localMemberName.resolution = null;
/*     */
            }
...
/* 853 */
                localMemberName.resolution = localLinkageError;
/* 854 */
                return localMemberName;
/*     */
            }
