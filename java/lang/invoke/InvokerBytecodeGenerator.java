...
/*  101 */
        this.localsMap = new int[paramInt];
/*      */
    }
...
/*  109 */
            this.localsMap[i] = (paramMethodType.parameterSlotCount() - paramMethodType.parameterSlotDepth(i));
/*      */
    }
...
/*  118 */
        int i = 0;
        for (int j = 0; i < this.localsMap.length; i++) {
/*  119 */
            this.localsMap[i] = j;
/*  120 */
            j += Wrapper.forBasicType(arrayOfName[i].type).stackSlots();
/*      */
        }
/*      */
    }
...
/*  150 */
        if (MethodHandleStatics.DUMP_CLASS_FILES) {
/*  151 */
            System.out.println("dump: " + paramString);
/*  152 */
...
/*  207 */
        if (MethodHandleStatics.DUMP_CLASS_FILES) str = str + " <<" + paramObject.toString() + ">>";
/*  208 */
...
/*  220 */
        for (CpPatch localCpPatch : this.cpPatches.values()) {
/*  221 */
            if (localCpPatch.index >= i)
/*  222 */
                throw new InternalError("in cpool[" + i + "]: " + localCpPatch + "\n" + Arrays.toString(Arrays.copyOf(paramArrayOfByte, 20)));
/*  223 */
            arrayOfObject[localCpPatch.index] = localCpPatch.value;
/*      */
        }
/*  225 */
...
/*  251 */
        return resolveInvokerMember(localClass, this.invokerName, this.invokerType);
/*      */
    }
...
/*      */
            default:
/*  387 */
                throw new InternalError("unknown type: " + paramChar);
/*      */
        }
/*  389 */
...
/*  419 */
        String str1 = "java/lang/" + localWrapper.wrapperType().getSimpleName();
/*  420 */
        String str2 = "valueOf";
/*  421 */
        String str3 = "(" + localWrapper.basicTypeChar() + ")L" + str1 + ";";
/*  422 */
        this.mv.visitMethodInsn(184, str1, str2, str3);
/*      */
    }
...
/*      */ {
/*  431 */
        Wrapper localWrapper = Wrapper.forWrapperType(paramClass);
/*  432 */
        String str1 = "java/lang/" + localWrapper.wrapperType().getSimpleName();
/*  433 */
        String str2 = localWrapper.primitiveSimpleName() + "Value";
/*  434 */
        String str3 = "()" + localWrapper.basicTypeChar();
/*  435 */
        this.mv.visitTypeInsn(192, str1);
/*  436 */
        this.mv.visitMethodInsn(182, str1, str2, str3);
/*      */
    }
...
/*      */
            case 'L':
/*  448 */
                if (VerifyType.isNullConversion(Object.class, paramClass))
/*  449 */ return;
/*  450 */
                if (isStaticallyNameable(paramClass)) {
/*  451 */
                    this.mv.visitTypeInsn(192, getInternalName(paramClass));
/*      */
                } else {
/*  453 */
...
/*  456 */
                    this.mv.visitMethodInsn(182, "java/lang/Class", "cast", "(Ljava/lang/Object;)Ljava/lang/Object;");
/*  457 */
...
/*      */
            case 'I':
/*  462 */
                if (!VerifyType.isNullConversion(Integer.TYPE, paramClass))
...
/*      */
            case 'J':
/*  466 */
                assert (paramClass == Long.TYPE);
/*  467 */
                return;
/*      */
            case 'F':
/*  469 */
                assert (paramClass == Float.TYPE);
/*  470 */
                return;
/*      */
            case 'D':
/*  472 */
                assert (paramClass == Double.TYPE);
/*  473 */
                return;
/*      */
            case 'E':
...
/*  475 */
        ...
        throw new InternalError("bad implicit conversion: tc=" + paramChar + ": " + paramClass);
/*      */
    }
...
/*      */
        int i;
/*  483 */
        switch (Wrapper.basicTypeChar(paramClass)) {
            case 'I':
/*  484 */
                i = 172;
                break;
...
/*      */
            default:
/*  491 */
                throw new InternalError("unknown return type: " + paramClass);
/*      */
        }
/*  493 */
        this.mv.visitInsn(i);
...
/*  497 */
        assert (VerifyAccess.isTypeVisible(paramClass, Object.class));
/*  498 */
...
/*  531 */
            LambdaForm.Name localName = this.lambdaForm.names[i];
/*  532 */
            MemberName localMemberName = localName.function.member();
/*      */ 
/*  534 */
            if (isSelectAlternative(localMemberName))
/*      */ {
/*  537 */
                emitSelectAlternative(localName, this.lambdaForm.names[(i + 1)]);
/*  538 */
                i++;
/*  539 */
            } else if (isStaticallyInvocable(localMemberName)) {
/*  540 */
                emitStaticInvoke(localMemberName, localName);
/*      */
            } else {
/*  542 */
...
/*      */ 
/*  548 */
            if ((i != this.lambdaForm.names.length - 1) || (i != this.lambdaForm.result))
/*      */ {
/*  550 */
                if (localName.type != 'V')
/*      */ {
/*  552 */
                    emitStoreInsn(localName.type, localName.index());
/*      */
                }
/*      */
            }
...
/*      */ {
/*  575 */
        MethodHandle localMethodHandle = paramName.function.resolvedHandle;
/*  576 */
        assert (localMethodHandle != null) : paramName.exprString();
/*  577 */
        this.mv.visitLdcInsn(constantPlaceholder(localMethodHandle));
/*  578 */
        this.mv.visitTypeInsn(192, "java/lang/invoke/MethodHandle");
/*      */ 
/*  589 */
        for (int i = 0; i < paramName.arguments.length; i++) {
/*  590 */
...
/*      */ 
/*  594 */
        MethodType localMethodType = paramName.function.methodType();
/*  595 */
        this.mv.visitMethodInsn(182, "java/lang/invoke/MethodHandle", "invokeBasic", localMethodType.basicType().toMethodDescriptorString());
/*      */
    }
...
/*      */ {
/*  607 */
        if (paramMemberName == null) return false;
/*  608 */
        if (paramMemberName.isConstructor()) return false;
/*  609 */
...
/*  623 */ return true;
/*  624 */
        if ((paramMemberName.isPublic()) && (isStaticallyNameable(localClass1)))
/*  625 */ return true;
/*  626 */
        return false;
/*      */
...
/*  654 */
        String str1 = getInternalName(paramMemberName.getDeclaringClass());
/*  655 */
...
/*  678 */
            this.mv.visitMethodInsn(refKindOpcode(b), str1, str2, str3);
/*      */
        } else {
/*  680 */
            str3 = MethodType.toFieldDescriptorString(paramMemberName.getFieldType());
/*  681 */
            this.mv.visitFieldInsn(refKindOpcode(b), str1, str2, str3);
/*      */
        }
/*      */
    }
    ...
    /*      */
/*  685 */   int refKindOpcode(byte paramByte) {
        switch (paramByte) {
            case 5:
/*  686 */
                return 182;
...
/*      */ {
/*  705 */
        return (paramMemberName != null) && (paramMemberName.getDeclaringClass() == MethodHandleImpl.class) && (paramMemberName.getName().equals("selectAlternative"));
/*      */
    }
...
/*      */ {
/*  724 */
        MethodType localMethodType = paramName1.function.methodType();
/*      */ 
/*  726 */
        LambdaForm.Name localName = (LambdaForm.Name) paramName2.arguments[0];
/*      */ 
/*  728 */
...
/*      */ 
/*  736 */
        this.mv.visitJumpInsn(160, localLabel1);
/*      */ 
/*  739 */
        MethodHandle localMethodHandle1 = (MethodHandle) paramName1.arguments[1];
/*  740 */
        emitPushArgument(paramName1, 1);
/*  741 */
        emitAstoreInsn(localName.index());
/*  742 */
        emitInvoke(paramName2);
/*      */ 
/*  745 */
        this.mv.visitJumpInsn(167, localLabel2);
/*      */ 
/*  748 */
        this.mv.visitLabel(localLabel1);
/*      */ 
/*  751 */
        MethodHandle localMethodHandle2 = (MethodHandle) paramName1.arguments[2];
/*  752 */
...
/*      */ 
/*  757 */
        this.mv.visitLabel(localLabel2);
/*      */
    }
...
/*      */ {
/*  766 */
        Object localObject = paramName.arguments[paramInt];
/*  767 */
        int i = paramName.function.parameterType(paramInt);
/*  768 */
        MethodType localMethodType = paramName.function.methodType();
/*  769 */
        if ((localObject instanceof LambdaForm.Name)) {
/*  770 */
            LambdaForm.Name localName = (LambdaForm.Name) localObject;
/*  771 */
            emitLoadInsn(localName.type, localName.index());
/*  772 */
            emitImplicitConversion(localName.type, localMethodType.parameterType(paramInt));
/*  773 */
        } else if (((localObject == null) || ((localObject instanceof String))) && (i == 76)) {
/*  774 */
            emitConst(localObject);
/*      */
        }
/*  776 */
        else if ((Wrapper.isWrapperType(localObject.getClass())) && (i != 76)) {
/*  777 */
            emitConst(localObject);
/*      */
        } else {
/*  779 */
            this.mv.visitLdcInsn(constantPlaceholder(localObject));
/*  780 */
            emitImplicitConversion('L', localMethodType.parameterType(paramInt));
/*      */
        }
/*      */
    }
...
/*  794 */
            LambdaForm.Name localName = this.lambdaForm.names[this.lambdaForm.result];
/*  795 */
            char c1 = Wrapper.basicTypeChar(this.invokerType.returnType());
/*      */ 
/*  798 */
            if (this.lambdaForm.result != this.lambdaForm.names.length - 1) {
/*  799 */
                emitLoadInsn(localName.type, this.lambdaForm.result);
/*      */
            }
/*      */ 
/*  805 */
            if (c1 != localName.type)
/*      */ {
/*  807 */
                if (c1 == 'L')
/*      */ {
...
/*  811 */
                        emitPrimCast(localName.type, c2);
/*      */
                    }
/*      */ 
/*  814 */
...
/*  817 */
                else if (localName.type != 'L')
/*      */ {
/*  819 */
                    emitPrimCast(localName.type, c1);
/*      */
                }
/*      */
                else {
...
/*  852 */
        Wrapper localWrapper1 = Wrapper.forBasicType(paramChar1);
/*  853 */
        Wrapper localWrapper2 = Wrapper.forBasicType(paramChar2);
/*  854 */
...
/*  859 */
        else if (localWrapper2.isSubwordOrInt())
/*      */ {
/*  861 */
            emitX2I(paramChar1);
/*  862 */
            if (localWrapper2.bitWidth() < 32)
/*      */ {
/*  864 */
                emitI2X(paramChar2);
/*      */
...
/*      */
                case 'J':
/*  871 */
                    if (paramChar2 == 'F') this.mv.visitInsn(137);
/*  872 */
                    else if (paramChar2 == 'D') this.mv.visitInsn(138);
                    else
/*  873 */           i = 1;
/*  874 */
                    break;
/*      */
                case 'F':
/*  876 */
                    if (paramChar2 == 'J') this.mv.visitInsn(140);
/*  877 */
                    else if (paramChar2 == 'D') this.mv.visitInsn(141);
                    else
/*  878 */           i = 1;
/*  879 */
                    break;
/*      */
                case 'D':
/*  881 */
                    if (paramChar2 == 'J') this.mv.visitInsn(143);
/*  882 */
                    else if (paramChar2 == 'F') this.mv.visitInsn(144);
                    else
/*  883 */           i = 1;
/*  884 */
                    break;
...
/*  926 */
            localStringBuilder.append(Wrapper.forBasicType(localClass).basicTypeChar());
/*  927 */
        localStringBuilder.append('_').append(Wrapper.forBasicType(paramMethodType.returnType()).basicTypeChar());
/*  928 */
...
/*      */ 
/*  942 */
        char c = LambdaForm.signatureReturn(paramString);
/*  943 */
        MethodType localMethodType = MethodType.methodType(LambdaForm.typeClass(c), MethodHandle.class);
/*      */ 
/*  945 */
        int i = LambdaForm.signatureArity(paramString);
/*  946 */
        for (int j = 1; j < i; j++) {
/*  947 */
            localMethodType = localMethodType.appendParameterTypes(new Class[]{LambdaForm.typeClass(paramString.charAt(j))});
/*      */
        }
/*  949 */
        InvokerBytecodeGenerator localInvokerBytecodeGenerator = new InvokerBytecodeGenerator("LFI", "interpret_" + c, localMethodType);
/*  950 */
...
/*  968 */
            localObject = this.invokerType.parameterType(i);
/*  969 */
            this.mv.visitInsn(89);
/*  970 */
            emitIconstInsn(i);
/*  971 */
            emitLoadInsn(Wrapper.basicTypeChar((Class) localObject), i);
/*      */ 
/*  973 */
            if (((Class) localObject).isPrimitive()) {
/*  974 */
                emitBoxing((Class) localObject);
/*      */
            }
/*  976 */
...
/*      */ 
/*  985 */
        Class localClass = this.invokerType.returnType();
/*  986 */
        if ((localClass.isPrimitive()) && (localClass != Void.TYPE)) {
/*  987 */
            emitUnboxing(Wrapper.asWrapperType(localClass));
/*      */
        }
/*      */ 
/*  991 */
        emitReturnInsn(localClass);
/*      */ 
/*  993 */
        classFileEpilogue();
/*  994 */
...
/*      */ 
/*  996 */
        Object localObject = this.cw.toByteArray();
/*  997 */
        maybeDump(this.className, (byte[]) localObject);
/*  998 */
        return localObject;
/*      */
    }
...
/* 1010 */
        String str = basicTypeCharSignature("invoke_", paramMethodTypeForm.erasedType());
/* 1011 */
...
/* 1042 */
                emitUnboxing(localObject3.wrapperType());
/* 1043 */
                emitPrimCast(localObject3.basicTypeChar(), ((Wrapper) localObject2).basicTypeChar());
/*      */
            }
/*      */ 
/*      */
...
/* 1049 */
        this.mv.visitMethodInsn(182, "java/lang/invoke/MethodHandle", "invokeBasic", str);
/*      */ 
/* 1052 */
...
/*      */ 
/* 1057 */
            emitPrimCast(((Wrapper) localObject1).basicTypeChar(), ((Wrapper) localObject2).basicTypeChar());
/* 1058 */
            emitBoxing(((Wrapper) localObject2).primitiveType());
/*      */
        }
/*      */ 
/* 1062 */
...
/*      */ 
/* 1070 */
        Object localObject1 = this.cw.toByteArray();
/* 1071 */
        maybeDump(this.className, (byte[]) localObject1);
/* 1072 */
        return localObject1;
/*      */
    }
...
/* 1083 */
                this.mv.visitLdcInsn(localObject.toString());
/* 1084 */
