...
/*  594 */
        if ((localMethodType.parameterCount() != i) || (isVarargsCollector()))
/*      */ {
/*  596 */
            return asType(MethodType.genericMethodType(i)).invokeWithArguments(paramArrayOfObject);
/*      */
        }
/*  598 */
        MethodHandle localMethodHandle = localMethodType.invokers().varargsInvoker();
/*  599 */
        return localMethodHandle.invokeExact(this, paramArrayOfObject);
/*      */
    }
...
/*  726 */
        return convertArguments(paramMethodType);
/*      */
    }
...
/*      */ {
/*  817 */
        asSpreaderChecks(paramClass, paramInt);
/*  818 */
        int i = this.type.parameterCount() - paramInt;
/*  819 */
        return MethodHandleImpl.makeSpreadArguments(this, paramClass, i, paramInt);
/*      */
    }
...
/*  827 */
        if ((paramClass !=[Ljava.lang.Object.class)&&(paramInt != 0)){
/*  828 */
            int j = 0;
/*  829 */
            Class localClass = paramClass.getComponentType();
/*  830 */
            for (int k = i - paramInt; k < i; k++) {
/*  831 */
                if (!MethodType.canConvert(localClass, type().parameterType(k))) {
/*  832 */
                    j = 1;
/*  833 */
                    break;
...
/*  836 */
            if (j != 0) {
/*  837 */
                ArrayList localArrayList = new ArrayList(type().parameterList());
/*  838 */
                for (int m = i - paramInt; m < i; m++) {
/*  839 */
                    localArrayList.set(m, localClass);
/*      */
                }
/*      */ 
/*  842 */
                asType(MethodType.methodType(type().returnType(), localArrayList));
/*      */
            }
/*      */
        }
...
/*  933 */
        MethodHandle localMethodHandle1 = this;
/*  934 */
        if (paramClass != type().parameterType(i))
/*  935 */ localMethodHandle1 = convertArguments(type().changeParameterType(i, paramClass));
/*  936 */
        MethodHandle localMethodHandle2 = ValueConversions.varargsArray(paramClass, paramInt);
/*  937 */
        return MethodHandles.collectArguments(localMethodHandle1, i, localMethodHandle2);
/*      */
    }
...
/*  947 */
            if (localClass.isAssignableFrom(paramClass)) return false;
/*      */
        }
/*  949 */
...
/*      */
        Class localClass;
/* 1208 */
        if ((localMethodType.parameterCount() == 0) || ((localClass = localMethodType.parameterType(0)).isPrimitive()))
/*      */ {
/* 1210 */
            throw MethodHandleStatics.newIllegalArgumentException("no leading reference parameter", paramObject);
/* 1211 */
        }
        paramObject = localClass.cast(paramObject);
/* 1212 */
        return bindReceiver(paramObject);
/*      */
    }
...
/*      */ {
/* 1232 */
        if (MethodHandleStatics.DEBUG_METHOD_HANDLE_NAMES) return debugString();
/* 1233 */
...
/*      */ {
/* 1263 */
        return MethodHandleImpl.makePairwiseConvert(this, paramMethodType, 0);
/*      */
    }
...
/*      */ {
/* 1295 */
        return MethodHandleImpl.makePairwiseConvert(this, paramMethodType, 1);
/*      */
    }
...
/*      */ {
/* 1301 */
        return rebind().bindArgument(paramInt, paramChar, paramObject);
/*      */
    }
...
/*      */ {
/* 1307 */
        return bindArgument(0, 'L', paramObject);
/*      */
    }
...
/*      */ {
/* 1318 */
        assert ((paramInt == 0) && (paramChar == 'L') && ((paramObject instanceof Unsafe)));
/* 1319 */
        MethodType localMethodType = this.type.dropParameterTypes(paramInt, paramInt + 1);
/* 1320 */
        LambdaForm localLambdaForm = this.form.bindImmediate(paramInt + 1, paramChar, paramObject);
/* 1321 */
        return copyWith(localMethodType, localLambdaForm);
/*      */
    }
...
/*      */ {
/* 1326 */
        throw new InternalError("copyWith: " + getClass());
/*      */
    }
...
/*      */ {
/* 1332 */
        return rebind().dropArguments(paramMethodType, paramInt1, paramInt2);
/*      */
    }
...
/*      */ {
/* 1338 */
        return rebind().permuteArguments(paramMethodType, paramArrayOfInt);
/*      */
    }
...
/* 1344 */
        MethodType localMethodType = type();
/* 1345 */
        LambdaForm localLambdaForm = reinvokerForm(localMethodType.basicType());
/*      */ 
/* 1347 */
        return BoundMethodHandle.bindSingle(localMethodType, localLambdaForm, this);
/*      */
    }
...
/*      */ {
/* 1352 */
        throw new InternalError("not a reinvoker MH: " + getClass().getName() + ": " + this);
/*      */
    }
...
/* 1360 */
        LambdaForm localLambdaForm = paramMethodType.form().cachedLambdaForm(8);
/* 1361 */
        if (localLambdaForm != null) return localLambdaForm;
/* 1362 */
        MethodHandle localMethodHandle = MethodHandles.basicInvoker(paramMethodType);
/*      */ 
/* 1365 */
...
/* 1370 */
        arrayOfName[k] = new LambdaForm.Name(NF_reinvokerTarget, new Object[]{arrayOfName[0]});
/* 1371 */
        Object[] arrayOfObject = Arrays.copyOfRange(arrayOfName, 0, i,[Ljava.lang.Object.class);
/* 1372 */
        arrayOfObject[0] = arrayOfName[k];
/* 1373 */
        arrayOfName[m] = new LambdaForm.Name(localMethodHandle, arrayOfObject);
/* 1374 */
        return paramMethodType.form().setCachedLambdaForm(8, new LambdaForm("BMH.reinvoke", i, arrayOfName));
/*      */
    }
...
/*      */ {
/* 1397 */
        if (this.form == paramLambdaForm) return;
/*      */ 
/* 1399 */
...
/* 1383 */
            throw MethodHandleStatics.newInternalError(localReflectiveOperationException1);
/*      */
        }
/*      */ 
/*      */
...
/* 1408 */
            throw MethodHandleStatics.newInternalError(localReflectiveOperationException2);
/*      */
        }
/*      */
    }
